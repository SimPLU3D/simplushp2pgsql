...
public class Simplushp2pgsql {
    ...
    private static class Credential {
        ...
        private Credential(String host, String db, String port, String user,
                String password, String table, String dirPath,
                boolean createtable) {
            this.host = host;
            this.db = db;
            this.port = port;
        ...
        public static Credential makeCredential(String dbCreds) {
            ...
            boolean createTable = creds.getProperty("createTable")
                    .equals("true");
            return new Credential(creds.getProperty("host"),
                    creds.getProperty("db"), creds.getProperty("port"),
                    creds.getProperty("user"), creds.getProperty("password"),
                    creds.getProperty("table"), creds.getProperty("shapesDir"),
                    createTable);
        }
    }
     ...
     * @param creds
     */
    public static void writeShapetoDB(Path dirPath, String run,
            Credential creds) {
        ...
        try (DirectoryStream<Path> stream = Files.newDirectoryStream(dirPath,
            ...
            for (Path p : stream) {
                dirs.add(p.toString());
                IPopulation<IFeature> shapes = ShapefileReader
                        .read(p.toString());
                for (IFeature f : shapes) {
                    DefaultFeature df = (DefaultFeature) f;
                    AttributeManager.addAttribute(df, "directory", imu,
                            "String");
                ...
                if (tableExists) {
                    ...
                    PostgisManager.insertInGeometricTable(creds.host,
                            creds.port, creds.db, creds.user, creds.password,
                            creds.table, shapes);
                } else {
                    // PostgisManager.saveGeometricTable("localhost", "5432",
                    // "iudf2", "imrandb", "imrandb", "asd", shapes, false);
                    PostgisManager.saveGeometricTable(creds.host, creds.port,
                            creds.db, creds.user, creds.password, creds.table,
                            shapes, false);
                    tableExists = true;
                }
    ...
    public static void main(String[] args) {
        Logger logger = Logger.getLogger(
                fr.ign.cogit.geoxygene.util.conversion.Reader.class.getName());
        ...
        if (!formatIsOk(run)) {
            System.out.println(
                    "incorrect format for run : it should be as YYMMDD like 170518");
            System.exit(0);
        }
